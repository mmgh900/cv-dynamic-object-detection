...
\node[stage, fill=mGreen!20, below=1.2cm of s1] (s4) {4.~Pick the movers\\{\scriptsize top 15\% noisiest}};
\node[stage, fill=mOrange!20, right=of s4]      (s5) {5.~Group \& clean\\{\scriptsize cluster + MAD filter}};
\node[stage, fill=mRed!15, right=of s5]         (s6) {6.~Draw the box\\{\scriptsize pad a bit, clamp}};
...
\node[owner, below=-1pt of s6] {\color{mGray}Riccardo};
...
on the object (\texttt{motion\_segmenter.cpp}). Orange/red =
grouping and boxing (\texttt{bbox\_estimator.cpp}). The name under
each stage is whoever wrote it.}
\end{figure}
...
\item Their score is at least $\max(6\,\text{px}, 3m)$, where $m$ is
...
\begin{tikzpicture}[font=\small]
  ...
  \draw[dashed, mOrange, thick] (3.9,0) -- (3.9,2.8) node[above, align=center] {\scriptsize cut-off\\\scriptsize $\max(6,3m)$};
...
rule (top 15\%) and an absolute floor ($\max(6, 3m)$ pixels).}
\end{figure}
...
6-pixel floor does the work; on slightly-panning scenes $3m$ grows
...
(MAD) in each axis. Any point more than $3\cdot\text{MAD}$ from the
...
\texttt{src/bbox\_estimator.cpp}   & Riccardo Pesce    & clustering, MAD, box \\
...
orange dots = all SIFT keypoints on frame 0, green lines =
trajectories of the points we classified as dynamic.
...
\paragraph{Squirrel (IoU 0.16).} The squirrel is tiny
($23\times31$\,px in a $259\times327$ image --- less than 1\% of the
picture). SIFT hardly finds any stable extrema on its fur. We end
up with only about 13 ``dynamic'' points, and a few of those are
actually wrong matches on nearby leaves, which drag the box outward
to $76\times52$. Our predicted box is in roughly the right place,
it's just the wrong size.

\paragraph{Sheep (IoU 0.36).} Three sheep standing behind a wire
fence. The fence posts look almost identical to each other; the
wool has lots of repeating texture. That means Lowe's ratio test
(correctly) rejects a lot of matches as ambiguous, and we end up
with too few surviving points to cover the full sheep group. The
box we draw covers one sheep well but misses the others.
...
\item Grow the predicted box using edge and colour cues --- Canny +
  morphological closing, SLIC superpixels, or GrabCut seeded by our
  current box.
\end{itemize}
